...
const NavbarSlider = ({ sideNav, setSideNav }: { sideNav: boolean, setSideNav: (sideNav: boolean) => void }) => {
    return (
        <div className='space-y-3 w-fit relative'>
            {
                Routes?.map((e, idx) =>
                    <Link href={`${e.route}`} onClick={() => setSideNav(!sideNav)} className={`flex gap-2 text-black hover:text-color font-semibold text-lg ${idx + 1 == 10 ? "" : ""}`} key={idx}>
